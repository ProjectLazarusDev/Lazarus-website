...
async function main() {
    ...
    for (let tokenId = 1; tokenId <= mintedCount; ++tokenId) {
        try {
            const response = await axios.get(`https://hfihu314z3.execute-api.us-east-1.amazonaws.com/collection/arb/bobots-genesis/${tokenId}/refresh`);
